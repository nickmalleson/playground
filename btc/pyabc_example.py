...
def distance(model_result_dict, observations_dict):
    ...
    model_result = model_result_dict['model_result']  # 'model_result' was the key in the dict defined in my_model
...
# The simulation result is already a dictionary, but the observations are not, so we make a dictionary on the fly)
good_result_distance = distance(good_result, {'observation': obs})
bad_result_distance = distance(bad_result, {'observation': obs})
