# Video game recommender — local web app
...
There is **no default user and no built-in data**. On the very first run there
are no users yet, so create one before starting the server:

```
python server.py --new-user alice
```

You'll be asked for a display name and for your favourite video games and
developers (one per line). Claude then builds a personalised starting profile —
six genres and 24 seed games tuned to those tastes — and saves it to
`users/alice/profile.json`. If you start the server before creating any user it
...
```
══════════════════════════════════════════════════════════
Video game recommender server
  working dir : /Users/.../game_recommender
  user        : alice  (Alice)
...
python server.py --port 5071         # different port
...
The app supports more than one player. Each user has their own **profile**
(favourite games and developers, genres and seed games — the personalised
"prompt") and their own **state** (current list, marks, history), stored under
`users/<name>/`.

```
python server.py                     # run as the last-used user
python server.py --user alice        # run as a specific user
...
- **Default behaviour** is to reuse whichever user you last ran. The most
  recent user is remembered in `last_user.txt`. With no remembered user and
  exactly one user on disk, that user is used; otherwise pick one with `--user`.
- **There is no built-in user.** Every profile is generated by Claude from the
  favourite games you supply via `--new-user`.
...
You'll be asked for a display name and for the new player's favourite games and
developers (one per line). Claude then builds a personalised starting profile —
six genres and 24 seed games tuned to those tastes — and saves it to
...
- `mark id='hollow-knight' patch={'like': 'love'}` — every time you mark a game
...
- **`users/<name>/profile.json`** — a user's favourites, genres and seed games (safe to hand-edit)
- **`users/<name>/state.json`** — a user's marks, history and current games list (safe to back up, delete, or hand-edit)
...
- `POST /api/fresh-picks` — generate replacements for marked games
- `GET  /api/test` — minimal Claude ping for connectivity check (also surfaced as the "Test Claude" button in the UI)

## Troubleshooting

**"Address already in use"** — port 5051 is taken. Run `python server.py --port 5071` (or any free port).
